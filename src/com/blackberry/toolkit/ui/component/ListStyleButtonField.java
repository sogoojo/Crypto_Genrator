...
public class ListStyleButtonField extends Field {
	...
	private static final int COLOR_BACKGROUND_FOCUS = 0x0399AA; // 0x7ABD3A;//0x186DEF;
	...
	public ListStyleButtonField(Bitmap icon, String label, Bitmap actionIcon,
		...
		_labelField = new MyLabelField(label, style, _drawPosition);
	...
	protected void paint(Graphics g) {
		// Left Bitmap
		if (_leftIcon != null) {
			g.drawBitmap(HPADDING, VPADDING, _leftIcon.getWidth(), _leftIcon
					.getHeight(), _leftIcon, 0, 0);
		}

		// Text
		try {
			g
					.pushRegion(_leftOffset, (getHeight() - _labelHeight) / 2,
							getWidth() - _leftOffset - _rightOffset,
							_labelHeight, 0, 0);
			_labelField.paint(g);
		...
		if (_actionIcon != null) {
			g.drawBitmap(getWidth() - HPADDING - _actionIcon.getWidth(),
					(getHeight() - _actionIcon.getHeight()) / 2, _actionIcon
							.getWidth(), _actionIcon.getHeight(), _actionIcon,
					0, 0);
		}
	}
	...
	protected void paintBackground(Graphics g) {
		...
		try {
			if (_drawPosition == 0) {
				...
				// g.setColor( background );
				// g.setColor(0x7ABD3A);
				// background = g.isDrawingStyleSet( Graphics.DRAWSTYLE_FOCUS )
				// ? 0x5D912C : 0x7ABD3A;
				background = g.isDrawingStyleSet(Graphics.DRAWSTYLE_FOCUS) ? 0x0399AA
						: 0x0399AA;
				g.setColor(background);
				g.fillRoundRect(0, 0, getWidth(), getHeight(), CORNER_RADIUS,
						CORNER_RADIUS);
	...
	private static class MyLabelField extends LabelField {
		...
		public MyLabelField(String text, long style, int drawPos) {
			super(text, style);
		...
		public void paint(Graphics g) {
			...
			try {
				...
				int color = 0;
				if (drawPos < 0)
					color = g.isDrawingStyleSet(Graphics.DRAWSTYLE_FOCUS) ? 0xffffff
							: 0x000000;
				else
					color = g.isDrawingStyleSet(Graphics.DRAWSTYLE_FOCUS) ? 0xffffff
							: 0x272727;
